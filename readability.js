...
var readability = {
  ...
  grabArticle: function () {
    ...
    let medium = (window.location.hostname.indexOf('medium.com') != -1) ? true : false;
  ...
  grabArticleText: function extract(node){
    ...
    for(var childIdx = 0; childIdx < node.childNodes.length; childIdx++){
      ...
      parentNodeName = node.nodeName.toLowerCase();
    ...
    nodeName = node.nodeName.toLowerCase();
     // Check if it's a code snippet and flag it 
     if(nodeName.match(/pre|code/)) {
      text = text.split('\n') 
