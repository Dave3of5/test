    ...
    return function textToNodes(text) {
      ...
      txt = txt.split(/[\s]+/g)
            ...
            .map((text, i) => {
              if(!inCode && text != '**CODE**') {
                return text;
              } else if(!inCode && text == '**CODE**'){
                  inCode = true;
              } else if (inCode && text != '**CODE**') {
                if(!snippetsUsed.includes(snippetNum)) {
                  snippetsUsed.push(snippetNum);
                  return codeRes[snippetNum];
                }
              } else if(inCode && text == '**CODE**') {
                inCode = false;
                snippetNum++;
      ...
      //     if(inCode) { 
      //       codeSnipNum++;
      //       codeTxt[codeSnipNum] = {id: 0, text: ''};
      //     }
      //   }
      //   if(inCode && node.innerText != '**CODE**') {
      //     codeTxt[codeSnipNum].text += node.innerText;
      //     codeTxt[codeSnipNum].id = i-1;
      //     deleteIdxs.push(i-1);
      //   }
      // });

      // deleteIdxs.forEach(i => {
      //   txt.splice(i, 1)
      // }
      // );

      // Object.keys(codeTxt).forEach(key => {
      //   console.log('txt[codeTxt[key].id]: ', txt[codeTxt[key].id])
      //   if(txt[codeTxt[key].id] != undefined) {
      //     txt[codeTxt[key].id].innerText = codeTxt[key].text;
      //   }
      // });

      //console.log('code texts: ', codeTxt);
      return txt; 
    };
  };
